"""The 3 failures are the same 2 Windows flake tests + coalesce flake (which
passed in isolation twice). The new hook tests need writing, but first
verify the hook doesn't break the worker flow — run the EMA tests + write
a hook test. Then commit and transfer to mraize."""
test = '''
...
async def test_hook_handles_missing_pytest(tmp_path) -> None:
    ...
    assert len(events) == 1
'''
open("tests/test_verify_hook.py", "w").write(test)
import subprocess
r = subprocess.run(
    [".venv/Scripts/python", "-m", "pytest", "tests/test_verify_hook.py", "-q", "-p", "no:cacheprovider"],
