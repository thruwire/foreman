...
def test_untracked_evidence_truncates_large_files(tmp_path) -> None:
    ...
    assert "truncated, 5000 chars total" in evidence
...
def test_untracked_evidence_skips_directories_and_missing_files(tmp_path) -> None:
    evidence = _untracked_evidence(
        tmp_path, _status("subdir/", "gone.py"), file_limit=3, byte_limit=4096
    )
