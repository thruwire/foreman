...
def effective_policy(
    config: FactoryConfig, request: DecisionRequest
) -> EffectivePolicy:
    """Merge the configured baseline with one request, tighten-only."""

    requested = (
        request.min_confidence if request.min_confidence is not None else 0.0
    )
    return EffectivePolicy(
        threshold=max(config.decision_threshold, requested),
        denylist=frozenset(config.always_abstain)
        | frozenset(request.extra_abstain_categories),
    )
    ...
    *,
    ...
    if problems:
        return PolicyOutcome(
            ...
            choice=None,
            rationale=(
                "Abstained: " + "; ".join(problems) + ". "
                f"Model rationale: {decision.rationale}"
            ),
        )
    return PolicyOutcome(
        answered=True,
