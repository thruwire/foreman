...
# Marker so the working-directory preamble is never prepended twice when a
# record is reused across runs.
_WORKSPACE_PREAMBLE_MARKER = "[foreman:hermes-working-directory]"


class HermesWorker:
    """A worker driving the local Hermes Agent CLI in headless single-query mode.

    Runs ``hermes chat -q <mission> -Q --format stream-json`` as a streaming
    subprocess. Hermes emits newline-delimited JSON events (system init,
    tool_use, tool_result, text); each line is decoded, tail-bounded into the
    worker record, and forwarded to the factory event stream so the supervisor
    sees tool activity, not just final text.

    Headless single-query mode has no live-turn input channel, so
    ``supports_steering`` is ``False``, ``steer`` always returns ``False``,
    and the policy falls back to stop/retry for this backend. Termination
    mirrors the OpenCode backend: SIGTERM to the process group, then SIGKILL
    after the graceful window (taskkill /T /F on Windows, which has no
    process groups).
    """
    ...
    def _pin_workspace(self, record: WorkerRecord, repository: Path) -> None:
        """Prepend a working-directory preamble to the mission (idempotent).
        ...
        """
        if record.mission.startswith(_WORKSPACE_PREAMBLE_MARKER):
            return
        repo_path = str(repository)
        record.mission = (
            f"{_WORKSPACE_PREAMBLE_MARKER}\n"
    ...
    async def run(
        ...
        # Reset all per-run state so a reused instance starts clean: a stale
        # termination reason or result exit code from a previous run must not
        # leak into this run's status mapping.
        self.process = None
        self._termination_reason = None
        self._result_exit_code = None
        record.status = WorkerStatus.RUNNING
        record.started_at = datetime.now(UTC)
        self._pin_workspace(record, repository)
        # Capability probing shells out; keep it off the event loop so one
        # slow `hermes chat --help` cannot stall concurrent workers. The
        # result is cached per instance, so this runs at most once.
        await asyncio.to_thread(self._probe_capabilities)
        try:
            ...
            # long-running worker would stream nothing until exit.
        ...
        try:
            ...
            # Drain both pipes before mapping the terminal status: hermes' own
            # `result` event can arrive just before process exit, and the
            # mapping below depends on it. The drain is bounded so a lingering
            # child holding the pipes open cannot stall the run; the finally
            # block still joins the readers.
            try:
                await asyncio.wait_for(
                    asyncio.gather(stdout_task, stderr_task, return_exceptions=True),
                    timeout=self.graceful_termination_seconds,
                )
            except TimeoutError:
    ...
    async def terminate(self, reason: str) -> None:
        ...
        if os.name == "nt":
            ...
            # asyncio.to_thread keeps the async loop unblocked.
